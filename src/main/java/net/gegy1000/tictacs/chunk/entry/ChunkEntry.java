...
public final class ChunkEntry extends ChunkHolder {
    ...
    public ChunkEntry(
            ...
            IListener levelUpdateListener,
            IPlayerProvider watchers
    ) {
    ...
    public boolean isTicking() {
        Either<Chunk, IChunkLoadingError> ticking = this.getTickingFuture().getNow(null);
        if (ticking == null) {
            return false;
        }
    ...
    public boolean isTickingEntities() {
        Either<Chunk, IChunkLoadingError> entityTicking = this.getEntityTickingFuture().getNow(null);
    ...
    public Chunk finalizeChunk(ServerWorld world, LongPredicate loadToWorld) {
        ...
        worldChunk.setLocationType(() -> ChunkHolder.getLocationTypeFromLevel(this.prevChunkLevel));
        worldChunk.postLoad();
    ...
    @Override
    @Deprecated
    public CompletableFuture<Either<IChunk, IChunkLoadingError>> func_219276_a(ChunkStatus status, ChunkManager tacs) {
        return tacs.func_219244_a(this, status);
    }

    @Override
    @Deprecated
    public CompletableFuture<Either<IChunk, IChunkLoadingError>> func_219301_a(ChunkStatus status) {
    ...
    @Override
    @Deprecated
    public CompletableFuture<Either<IChunk, IChunkLoadingError>> func_225410_b(ChunkStatus status) {
        ChunkStep step = ChunkStep.byStatus(status);
